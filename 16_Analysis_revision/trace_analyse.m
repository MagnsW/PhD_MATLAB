...
output.freq_analysis.phase_unwrapped = unwrap(output.freq_analysis.phase);%unwrap added
% phase_diff = diff(output.freq_analysis.phase_unwrapped, 1, 2);
phase_diff = output.freq_analysis.phase_unwrapped; %just for testing. Delete and uncomment previous line.
% phase_diff(phase_diff > pi) = phase_diff(phase_diff > pi) - 2*pi;
% phase_diff(phase_diff < -pi) = phase_diff(phase_diff < -pi) + 2*pi;
output.freq_analysis.phase_diff = phase_diff; %unwrap added
