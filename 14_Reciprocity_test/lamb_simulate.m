...
dv_dy = 1+0.0015*cumtrapz(mexihat(-5, 5, Ny)); %Integrated mexican hat
dv_dy = flip(dv_dy);
...
% sensor.mask = [source_x_pos, 1175 - source_radius, source_x_pos, 1175 + source_radius].';
sensor.mask = [source_x_pos, 640 - source_radius, source_x_pos, 640 + source_radius].'; % Halfway
...
% input_args = {'PlotSim', true, 'DataCast', 'single', 'PMLSize', PML_size}; % rerun4
input_args = {'PlotScale', 'auto', 'PlotSim', true, 'DataCast', 'single', 'PMLAlpha', [1 1], 'PMLSize', PML_size, 'RecordMovie', false, 'MovieName', 'periodic_movie'}; % rerun4
