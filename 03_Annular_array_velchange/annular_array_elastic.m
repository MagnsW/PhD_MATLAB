function [sensor_data] = annular_array_elastic(Nx_thickness)
...
signal_offsets = zeros(1, num_source_nodes);
t_delay = 100e-9 %60e-9; %ns
s_delay = floor(t_delay/dt);
signal_offsets(1, 76:175) = s_delay;
source.ux = source_mag * toneBurst(1/kgrid.dt, source_freq, source_cycles, 'SignalOffset', signal_offsets);
% for idx = 1:num_source_nodes
%     source.ux(idx,:) = source_mag * toneBurst(1/kgrid.dt, source_freq, source_cycles);
% end
% for idx2 = 76:175
%     source.ux(idx2,:) = source_mag * toneBurst(1/kgrid.dt, source_freq/2, source_cycles/2);
% end
...
'PMLSize', PML_size,'DisplayMask', source.u_mask, 'RecordMovie', false, 'MovieName', 'annular_array'};
